...
/// struct containing a new user's info
#[derive(serde::Serialize, serde::Deserialize, Debug, Clone, Default)]
pub struct UserInformation {
    ...
    pub password: String,
    /// uuids are runtime generated by the new function
    pub uuid: String,
...
pub fn decrypt_aes256(string_to_be_decrypted: &str) -> Result<String, FromUtf8Error> {
...
#[inline]
pub fn pass_hash_match(to_be_verified: String, encoded: String) -> bool {
...
pub fn register(username: String, passw: String) -> Result<()> {
    ...
    file.write_all(user_information.as_bytes())?;
    ...
    if !file_ln.is_empty() {
        for item in file_ln {
            ...
            if !item.trim().is_empty() {
                let decrypted_line = decrypt_aes256(&item)?;
